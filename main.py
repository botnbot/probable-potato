...
if is_sort_by_date =='да':
    print('Операции сортируются по дате')
    order = ''
    ...
    print('Операции не сортируются по дате')
...
if filtered_by_string:
    ...
    for transaction in filtered_by_string:
        frm = str(mask_account_card(transaction["from"])) + ' -> ' if "from" in transaction else ''
        to = mask_account_card(transaction["to"]) if "to" in transaction else ''
        date = get_data(str(transaction["date"])) if "date" in transaction else ''
        description = transaction['description'] if 'description' in transaction else ''
        amount = transaction["operationAmount"]["amount"] if "operationAmount" in transaction else ''
        name = transaction["operationAmount"]["currency"]["name"] if "operationAmount" in transaction else ''
